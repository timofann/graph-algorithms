...
using namespace s21;
#define ANTS_NUM 5 // количество муравьев в одном городе
...
#define RHO 0.5 // коэффициент забывчивости [0, 1] (1 - не обращаем внимания на других муравьёв, 0 - помним всё)
#define QV 100 // количество фермента, которое муравей может распределить по пути
...
restart_ants(const Graph &graph, std::vector<Ant>& ants) {
	for (int i = 0; i < graph.vertices_cnt_ * ANTS_NUM; ++i) {
...
static std::vector<int>
get_available_paths(const Graph &graph, Ant ant) {
...
choose_path(const Graph &graph, const std::vector<std::vector<double>>& pheromone_lvl,
			const std::vector<int>& available_paths, Ant ant) {
...
//	std::vector<int> cities_to_start_from = euler_graph_check(graph);
	std::vector<Ant> ants = std::vector<Ant>(graph.vertices_cnt_ * ANTS_NUM,
